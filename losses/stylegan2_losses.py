...
def d_logistic(real_images, generator, discriminator, z_dim):
...
def d_logistic_r1_reg(real_images, generator, discriminator, z_dim):
    batch_size = tf.shape(real_images)[0]
    z = tf.random.normal(shape=[batch_size, z_dim], dtype=tf.float32)
    ...
    with tf.GradientTape() as r1_tape:
        ...
        real_loss = tf.reduce_sum(discriminator(real_images, training=True))
...
def g_logistic_non_saturating(real_images, generator, discriminator, z_dim):
    batch_size = tf.shape(real_images)[0]
    z = tf.random.normal(shape=[batch_size, z_dim], dtype=tf.float32)
...
def g_logistic_ns_pathreg(
    ...
    # forward pass
    fake_images, w_broadcasted = generator(z, ret_w_broadcasted=True, training=True)
    fake_scores = discriminator(fake_images, training=True)
    g_loss = tf.math.softplus(-fake_scores)
    ...
    with tf.GradientTape() as pl_tape:
        pl_tape.watch(pl_z)
        pl_fake_images, pl_w_broadcasted = generator(
            pl_z, ret_w_broadcasted=True, training=True
        )
